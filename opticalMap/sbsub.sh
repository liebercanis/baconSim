...
for a in `seq $FIRST $LAST` ; do  
  export macro =baconMap$a.mac
  export logfile =baconMap$a.log
  echo macro=$macro logfile=$logfile 
  echo "srun --output=$projectdir/$logfile.log -n 1  -C haswell shifter --env-file=$projectdir/MaGeEnvirons $projectdir/bin/Linux-g++/MaGe $projectdir/$macro"
  #srun --output=$projectdir/$logfile -n 1  -C haswell shifter --env-file=$projectdir/MaGeEnvirons $projectdir/bin/Linux-g++/MaGe $projectdir/$macro
done

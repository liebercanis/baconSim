...
class TGeDet: public TNamed {
  public:
    ...
    // handy printing
    TString getGlobalString() {
      TString s;
      s.Form("(%.1f,%.1f,%.1f)",global.X(),global.Y(),global.Z());
      return s;
    }

    TString getPminString() {
      TString s;
      s.Form("(%.1f,%.1f,%.1f)",pMin.X(),pMin.Y(),pMin.Z());
      return s;
    }

    TString getPmaxString() {
      TString s;
      s.Form("(%.1f,%.1f,%.1f)",pMax.X(),pMax.Y(),pMax.Z());
      return s;
    }
